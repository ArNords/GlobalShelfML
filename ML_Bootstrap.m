...
clear
analysisType = 'Bootstrap'; % Define type for this script version
...
if RI == 1
    RandIterations = 300; % How many iterations?
end

% The total number of models is (560 or RandIterations*numPermutations)+(RandIterations*numPermutations*7)
...
for i = nCombinations
    ...
    while DO < 0.6 || DO > 0.95
        ...
        for j = trainIndices
            ...
            TX = [];
            TX =  bootstrp(20,@median,DataPo(:,[4 5 6 7 8 9 10]));
            TrainY = bootstrp(20,@median,DataPo(:,De));
            Y1 = [Y1; TrainY];
            X1 = [X1; TX];
        ...
        for j = testIndices
            ...
            TX = [];
            TX = bootstrp(20,@median,DataPo(:,[4 5 6 7 8 9 10]));      
            TestY = bootstrp(20,@median,DataPo(:,De));
            Y2 = [Y2; TestY];
            X2 = [X2; TX];
...
resultsData.kernelFunctionsUsed = KF1;
resultsData.epsilonAndBoxConstraints = KF2;
